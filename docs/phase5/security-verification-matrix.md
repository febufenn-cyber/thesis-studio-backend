...
| File uploads | Upload size/type/ZIP controls and deterministic parser run before use. | Phase 1 upload/preflight pipeline; commercial size entitlement | Phase 1 ingestion tests | Malware scanner deployment evidence — Operations owner |
...
| Error handling | User-safe errors do not expose stack traces or provider credentials. | Central exception handlers | Regression suite | Staging fault injection — Reliability owner |
| Dependencies | Dependency and static security scans run on PRs and weekly. | `phase5-security.yml` | pip-audit and Bandit | Triage evidence and exception expiry — Security owner |
...
| Release integrity | Deployments use exact SHA, immutable image, migration validation, canary and smoke checks. | `phase5-release.yml` | Release workflow validation | Protected environment approval evidence — Operations owner |
...
- Review of Cloudflare, PostgreSQL and R2 IAM
- Review of upload malware-scanning deployment
