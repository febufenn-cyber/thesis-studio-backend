# Robofox Thesis Studio — Phase 5 Production Topology

## Goal

Reduce the blast radius of application, AI, PDF, database, storage and deployment failures without introducing premature microservices.
...
6. At least two application instances sit behind health-aware routing before institutional uptime commitments.
7. Workers use expiring leases and idempotent operation identifiers.
8. Secrets are injected through the deployment environment or mounted secret files, never stored in the repository or database.
...
Exports and jobs retain version/checksum context so a final output can be investigated and reproduced.
