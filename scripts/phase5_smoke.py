#!/usr/bin/env python3
"""Read-only post-deploy smoke test for release candidates."""
...
def fetch_json(base: str, path: str, timeout: float) -> tuple[int, dict]:
    request = urllib.request.Request(
        base.rstrip("/") + path,
        headers={"User-Agent": "robofox-release-smoke/2"},
    )
    try:
...
def main() -> int:
    ...
    checks: list[dict] = []
    failures: list[str] = []
    for path in ("/healthz", "/readyz", "/status", "/meta/release"):
    ...
    ready = next((row["payload"] for row in checks if row["path"] == "/readyz"), {})
    ...
    if health.get("phase") != "commercial_reliability_security_scale":
        failures.append("health endpoint is not reporting the commercial reliability phase")

    readiness_checks = ready.get("checks", {})
    for key in ("database", "migration", "worker", "pdf_stack", "storage", "email", "malware_scanner"):
        if not readiness_checks.get(key, {}).get("ok"):
            failures.append(f"readiness component {key!r} is not healthy")
    if ready.get("schema_version") != release.get("schema_version"):
        failures.append("readiness and release endpoints disagree on schema version")
