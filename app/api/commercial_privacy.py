...
from app.models.institutional_governance import RetentionPolicy
...
async def _grace_days(db: AsyncSession, institution_id: UUID) -> int:
    ...
    if row is None:
        return get_settings().DELETION_GRACE_DAYS
    return int((row.policy or {}).get("deletion_grace_days", get_settings().DELETION_GRACE_DAYS))
...
async def create_lifecycle_request(
    ...
    if body.request_type == "project_delete":
        ...
        access = await require_project_capability(
            db, body.project_id, current_user, "data.request_deletion_self"
        )
    ...
    if duplicate:
        return {
            ...
            "execute_after": duplicate.execute_after,
            "duplicate": True,
        }

    now = datetime.now(timezone.utc)
    grace = await _grace_days(db, institution_id)
    execute_after = now + timedelta(days=grace) if body.request_type != "data_export" else None
    row = DataLifecycleRequest(
        ...
        completed_at=now if body.request_type == "data_export" else None,
        result={
            "active_access": (
                "scheduled for removal"
                if body.request_type != "data_export"
                else "use authenticated export endpoint"
            ),
    ...
    if body.request_type != "data_export":
        await enqueue_job(
            ...
            idempotency_key=f"data-lifecycle:{row.id}",
            max_attempts=10,
        )
    await db.commit()
    return {
    ...
    rows = list(
        (
            await db.execute(
                ...
                .order_by(DataLifecycleRequest.requested_at.desc())
            )
        ).scalars()
    )
    return [
...
async def cancel_lifecycle_request(
    ...
    row = (
        await db.execute(
            select(DataLifecycleRequest).where(
                DataLifecycleRequest.id == request_id,
                DataLifecycleRequest.user_id == current_user.id,
            )
        )
    ).scalar_one_or_none()
...
async def current_privacy_notice(
    ...
    row = (
        await db.execute(
            ...
            .where(
                PrivacyNoticeVersion.audience == audience,
                PrivacyNoticeVersion.jurisdiction == jurisdiction,
                PrivacyNoticeVersion.state == "published",
                PrivacyNoticeVersion.effective_from <= datetime.now(timezone.utc),
            )
            .order_by(PrivacyNoticeVersion.version.desc())
            .limit(1)
        )
    ).scalar_one_or_none()
    if row is None:
    ...
    notice = (
        await db.execute(
            select(PrivacyNoticeVersion).where(
                PrivacyNoticeVersion.id == body.notice_version_id,
                PrivacyNoticeVersion.state == "published",
            )
        )
    ).scalar_one_or_none()
...
async def create_privacy_notice(
    ...
    latest = (
        await db.execute(
            select(PrivacyNoticeVersion.version)
            .where(
                PrivacyNoticeVersion.audience == body.audience,
                PrivacyNoticeVersion.jurisdiction == body.jurisdiction,
            )
            .order_by(PrivacyNoticeVersion.version.desc())
            .limit(1)
        )
    ).scalar_one_or_none()
    row = PrivacyNoticeVersion(
...
async def publish_privacy_notice(
    institution_id: UUID,
    notice_id: UUID,
    current_user: CurrentUser,
    current_session: CurrentApplicationSession,
    ...
    await require_recent_reauthentication(current_session)
    row = (
        await db.execute(select(PrivacyNoticeVersion).where(PrivacyNoticeVersion.id == notice_id))
    ).scalar_one_or_none()
    if row is None:
...
async def add_data_inventory(
    institution_id: UUID,
    body: DataInventoryCreate,
    current_user: CurrentUser,
    db: AsyncSession = Depends(get_db),
) -> dict:
    await require_institution_capability(db, institution_id, current_user, "privacy.manage")
    purpose = (
        await db.execute(
            select(ProcessingPurpose).where(
                ProcessingPurpose.key == body.purpose_key,
                ProcessingPurpose.state == "active",
            )
        )
    ).scalar_one_or_none()
    if purpose is None:
        raise HTTPException(status_code=422, detail="Unknown processing purpose")
    row = DataInventoryRecord(
        institution_id=institution_id,
        data_category=body.data_category,
        purpose_key=body.purpose_key,
        subject_owner=body.subject_owner,
        storage_system=body.storage_system,
        retention_rule=body.retention_rule,
...
async def institution_data_map(
    institution_id: UUID,
    current_user: CurrentUser,
    db: AsyncSession = Depends(get_db),
) -> dict:
    ...
    inventory = list(
        (
            await db.execute(
                select(DataInventoryRecord)
                .where(
                    DataInventoryRecord.institution_id == institution_id,
                    DataInventoryRecord.state == "active",
                )
                .order_by(DataInventoryRecord.data_category)
            )
        ).scalars()
    )
    subprocessors = list(
        (
            await db.execute(
                select(SubprocessorRecord)
                .where(SubprocessorRecord.state == "active")
                .order_by(SubprocessorRecord.name)
            )
        ).scalars()
    )
    return {
        ...
        "inventory": [
            {
                ...
                "owner": row.subject_owner,
                "storage": row.storage_system,
                "retention": row.retention_rule,
                "shared_with": row.shared_with,
                "deletion_path": row.deletion_path,
                "durable_class": row.durable_class,
            }
