...
async def feature_enabled(
    ...
    *,
    ...
    flag = (
        await db.execute(select(FeatureFlag).where(FeatureFlag.key == key, FeatureFlag.state == "active"))
    ).scalar_one_or_none()
    if flag is None:
        return False
        ...
        (
            await db.execute(
                select(RolloutAssignment).where(
                    ...
                    or_(
                        RolloutAssignment.user_id == user_id if user_id else RolloutAssignment.user_id.is_(None),
                        RolloutAssignment.institution_id == institution_id if institution_id else RolloutAssignment.institution_id.is_(None),
                    ),
                )
            )
        ).scalars()
    )
    # User overrides tenant overrides global assignments, newest wins within scope.
    assignments.sort(
        key=lambda row: (
