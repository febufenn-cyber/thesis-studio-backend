"""ASME (current) reference style — a NUMBERED-family CitationStyle.

Follows the American Society of Mechanical Engineers reference format used in
mechanical engineering: numbered references in order of appearance, with the
ASME journal form ``Author, Year, "Title," Journal (italic), Volume(Issue),
pp. Pages.`` The stored author string is used verbatim (author-name reformatting
is out of scope, as with IEEE), and only ``italic`` emphasis is available on the
canonical Run. Missing required fields raise MissingCitationField — bibliographic
...
class ASMEStyle:
    ...
    def format_reference(self, source: SourceLike, ordinal: int | None = None) -> list[Run]:
        ...
        if kind in ("book", "translated_book"):
            runs = [Run(text=f"{prefix}{f['author']}, {f['year']}, ")]
        ...
        if kind == "chapter_in_collection":
            return [
                Run(text=f'{prefix}{f["author"]}, {f["year"]}, "{f["title"]}," '),
                Run(text=f["container"], italic=True),
                Run(text=(
                    f", {f['editor']}, ed., {f['publisher']}, pp. {f['pages']}."
                )),
            ]
        if kind in ("journal", "journal_db"):
            runs = [
                Run(text=f'{prefix}{f["author"]}, {f["year"]}, "{f["title"]}," '),
                Run(text=f["container"], italic=True),
                Run(text=f", {f['volume']}({f['number']}), pp. {f['pages']}."),
            ]
            if kind == "journal_db":
