            ...
            <div style={S.meter}>
              <div
                style={{
                  ...
                  background: r.status === "verified" ? "#1F7A4D" : r.status === "unverifiable" ? "#6E655A" : "#9A6A00",
                }}
              />
            </div>
          )}
    ...
    <div>
      ...
      {checks.map((c) => (
        <div key={c.h} style={S.check}>
          <div style={{ ...S.checkIcon, background: c.ok ? "#1F7A4D" : "#9A6A00" }}>{c.ok ? "✓" : "!"}</div>
          <div>
...
const ErrorNote = ({ text }: { text: string }) => (
  <div style={{ color: "#B3362C", fontSize: 12.5, padding: "8px 0" }}>{text}</div>
);

/* ---------------- styles ---------------- */

const S: Record<string, React.CSSProperties> = {
  panel: { width: 396, borderLeft: "1px solid #DCD3C5", background: "#fff", height: "100%", overflowY: "auto", fontFamily: "'Source Sans 3', 'Inter', system-ui, sans-serif" },
  tabs: { display: "flex", gap: 2, padding: "10px 12px 0", borderBottom: "1px solid #DCD3C5", position: "sticky", top: 0, background: "#fff" },
  tab: { flex: 1, padding: "9px 4px", border: 0, background: "transparent", borderBottom: "2px solid transparent", fontWeight: 600, fontSize: 12.5, color: "#6E655A", cursor: "pointer" },
  ...
  select: { fontFamily: "inherit", fontSize: 12, border: "1px solid #DCD3C5", background: "#fff", borderRadius: 7, padding: "5px 6px" },
  btn: { display: "inline-flex", alignItems: "center", gap: 6, padding: "7px 12px", borderRadius: 9, border: "1px solid #DCD3C5", background: "#fff", color: "#1C1917", fontWeight: 600, fontSize: 12.5, cursor: "pointer" },
  btnPri: { background: "#1F4D3A", color: "#fff", borderColor: "#1F4D3A" },
  statement: { border: "1px solid #DCD3C5", borderRadius: 11, background: "#F4EFE6", padding: "13px 14px", fontSize: 13, lineHeight: 1.62, marginTop: 10 },
  meta: { fontSize: 11, color: "#6E655A", fontFamily: "'IBM Plex Mono', ui-monospace, monospace", marginTop: 10 },
  ...
  meter: { height: 9, borderRadius: 6, background: "#F4EFE6", border: "1px solid #DCD3C5", overflow: "hidden", marginTop: 8 },
  meterFill: { height: "100%", borderRadius: 6, transition: "width .6s ease" },
  finding: { display: "flex", gap: 9, border: "1px solid #DCD3C5", borderRadius: 10, padding: "10px 11px", marginBottom: 8 },
  sevDot: { width: 8, height: 8, borderRadius: "50%", marginTop: 5, flex: "none" },
  assert: { background: "#E8EFE9", color: "#1F4D3A", borderRadius: 10, padding: "10px 12px", fontSize: 12, fontWeight: 600, marginBottom: 14, lineHeight: 1.5 },
  check: { display: "flex", alignItems: "center", gap: 11, padding: "11px 0", borderBottom: "1px solid #DCD3C5" },
  ...
  modal: { background: "#fff", border: "1px solid #DCD3C5", borderRadius: 16, width: 560, maxWidth: "92vw", maxHeight: "86vh", overflow: "hidden", display: "flex", flexDirection: "column" },
  modalHead: { display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 18px", borderBottom: "1px solid #DCD3C5" },
  iconBtn: { width: 34, height: 34, borderRadius: 9, border: "1px solid #DCD3C5", background: "#fff", cursor: "pointer" },
  search: { display: "flex", alignItems: "center", gap: 9, border: "1px solid #DCD3C5", borderRadius: 10, padding: "9px 12px", background: "#F4EFE6" },
  searchInput: { border: 0, background: "transparent", outline: "none", fontFamily: "inherit", fontSize: 14, flex: 1 },
  cand: { border: "1px solid #DCD3C5", borderRadius: 11, padding: "11px 12px", marginTop: 10, display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center" },
};
